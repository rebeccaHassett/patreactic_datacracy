...
def main():
    ...
    if(args.state_number == 1):
        ...
        with open("clean_data/North_Carolina_U.S_Congressional_Districts_Geography_CLEAN.json") as f:
            district_features = json.load(f)["features"]

        with open("clean_data/nc_precincts_CLEAN.json") as f:
            precinct_features = json.load(f)["features"]

        complete_precinct_features = map_precincts_to_districts(district_features, precinct_features)

        original_precinct_file = "clean_data/nc_precincts_CLEAN.json"
        new_precinct_file = "mapped_data/nc_precincts_NEW.json"
